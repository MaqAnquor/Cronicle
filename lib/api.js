...
module.exports = Class.create({
	...
	requireValidEventParams: function(params, plugin, callback) {
		// Only allow Event params that are defined by the admin-controlled Plugin.
		// This prevents arbitrary param keys from becoming child environment variables.
		var allowed_params = Object.create(null);
		if (plugin.params) plugin.params.forEach( function(param) {
			allowed_params[ param.id ] = true;
		} );
		
		for (var key in params) {
			if (!Object.prototype.hasOwnProperty.call(params, key)) continue;
			if (!allowed_params[key]) {
				this.doError('event', "Unknown Plugin parameter: " + key, callback);
				return false;
			}
		}
