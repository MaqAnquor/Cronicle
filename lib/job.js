...
module.exports = Class.create({
	...
	launchJob: function(event, callback) {
		...
		function(err) {
			...
			for (var idx = 0, len = servers.length; idx < len; idx++) {
				...
				// Only allow params defined by the admin-controlled Plugin schema,
				// so arbitrary Event / API keys cannot become child environment variables.
				var supplied_params = Tools.isaHash(event.params) ? event.params : {};
				job.params = {};
				
				// Plugin params may have changed outside of the Event, so recopy
				// missing values, and always use the trusted value for hidden params.
				if (plugin.params) plugin.params.forEach( function(param) {
					if (
						(param.type != 'hidden') &&
						Object.prototype.hasOwnProperty.call(supplied_params, param.id)
					) {
						job.params[ param.id ] = supplied_params[ param.id ];
					}
					else {
				...
				if (hook_text_templates[hook_data.action]) {
					hook_data.text = hook_data.content = hook_data.message = Tools.sub( hook_text_templates[hook_data.action], hook_data );
	...
	handleChildResponse: function(job, worker, data) {
		...
		// disallow update_event unless specifically enabled in config
		if (data.update_event && !this.server.config.get('allow_event_updates_from_jobs')) {
			delete data.update_event;
		}
		
		// prevent xss in custom html report
		if (data.html && data.html.content) {
			data.html.content = sanitizeHtml( data.html.content, this.server.config.get('sanitize_html_config') || {
				"allowedTags": [ 
	...
	uploadJobLog: function(job, callback) {
		...
		if (this.multi.master) {
			...
			fs.stat( job.log_file, function(err, stats) {
				...
				var temp_file = job.log_file + '.gz';
				var temp_stream = fs.createWriteStream( temp_file );
				
				stream.pipe( gzip ).pipe( temp_stream ).on("finish", function() {
					var new_stream = fs.createReadStream( temp_file );
					
					self.storage.putStream( path, new_stream, function(err) {
	...
	fetchStoreJobLog: function(job) {
		...
		var api_url = this.getWorkerServerBaseAPIURL( slave.hostname, slave.ip ) + '/app/fetch_delete_job_log';
	...
	finishJob: function(job) {
		...
		if (hook_text_templates[hook_action]) {
			hook_data.text = hook_data.content = hook_data.message = Tools.sub( hook_text_templates[hook_action], hook_data );
		...
		// check for special server shutdown situation
		if (job.special_shutdown && job.catch_up && job.source) this.runJobAgain(job);
	},
	
	runJobAgain: function(old_job) {
		// launch custom new job from completed one
		var self = this;
		
		this.storage.listFind( 'global/schedule', { id: old_job.event }, function(err, event) {
			if (err || !event) {
				var err_msg = "Failed to rerun job: Event not found: " + old_job.event;
			...
			if (old_job.api_key) job.api_key = old_job.api_key;
			if (old_job.username) job.username = old_job.username;
			if (old_job.chain_data) {
				job.chain_data = old_job.chain_data;
				job.chain_code = old_job.code || 0;
				job.chain_description = old_job.description || '';
				job.source_event = old_job.event;
			}
			
			self.logDebug(6, "Rerunning job: " + job.title, job);
			
			self.launchOrQueueJob( job, function(err, jobs_launched) {
				if (err) {
					var err_msg = "Failed to rerun job: " + job.title + ": " + err.message;
					self.logError('scheduler', err_msg);
					self.logActivity( 'warning', { description: err_msg } );
					if (job.notify_fail) {
		...
		var finish = function(err, stdout, stderr) {
			...
			for (var id in self.activeJobs) {
				...
				if (pids[ job.pid ] && (now - job.time_start >= job_startup_grace)) {
					...
					// walk each generation explicitly, so we don't mutate the list we're scanning
					var levels = 0;
					var family = [ job.pid ];
					var seen = {};
					seen[ job.pid ] = 1;
